...
public class ELinkedList<E> extends AbstractSequentialList<E> implements
    ...
    private static final class LinkIterator<ET> implements ListIterator<ET> {
        ...
        final ELinkedList<ET> list;

        Link<ET> link, lastLink;

        LinkIterator(ELinkedList<ET> object, int location) {
            list = object;
        ...
        public ET next() {
            if (expectedModCount == list.modCount) {
                ELinkedList.Link<ET> next = link.next;
    ...
    private class ReverseLinkIterator<ET> implements Iterator<ET> {
        ...
        private final ELinkedList<ET> list;

        private Link<ET> link;

        private boolean canRemove;

        ReverseLinkIterator(ELinkedList<ET> linkedList) {
            super();
            list = linkedList;
            expectedModCount = list.modCount;
            link = list.voidLink;
    ...
    public Object clone() {
        try {
            ELinkedList<E> l = (ELinkedList<E>) super.clone();
            l.size = 0;
